...
class TestCalculate:
    ...
    def test_ErrorDuringCalculation(self, working_dir):
        ...
        with pytest.raises(
            Exception, match=re.escape("Errors were encountered while calculating files.")
        ):
            Snapshot.Calculate(
                dm_mock,
                [working_dir / "one"],
    ...
    def test_ErrorDuringHashing(self, working_dir):
        ...
        with pytest.raises(
            Exception, match=re.escape("Errors were encountered while hashing files.")
        ):
            Snapshot.Calculate(
                dm_mock,
                [working_dir / "one"],
    ...
    def test_CalculateOverlapError(self, mocked_is_dir, mocked_exists):
        ...
        with pytest.raises(
            Exception,
            match=re.escape(
                "The input '{}' overlaps with '{}'.".format(Path("one/two/three"), Path("one"))
            ),
        ):
            Snapshot.Calculate(
                mock.MagicMock(),
    ...
    def test_UnsupportedFileType(self, working_dir):
        ...
        os.symlink(
            working_dir / "two" / "Dir1", working_dir / "two" / "symDir", target_is_directory=True
        )
...
class TestDiff:
            ...
            Snapshot.Node.Create(
                {
                    ...
                    Path("two")
                    / "dir_with_content"
                    / "child1": ("hash(dir_with_content/child1)", 2001),
                    Path("two")
                    / "dir_with_content"
                    / "child2": ("hash(dir_with_content/child2)", 2002),
                    Path("two")
                    / "nested"
                    / "one"
                    / "two"
                    / "file1": ("hash(nested/one/two/file1)", 20001),
                    Path("two")
                    / "nested"
                    / "one"
                    / "two"
                    / "file2": ("hash(nested/one/two/file2)", 20002),
                    Path("three"): None,
                },
            ),
        )
    ...
    def test_RemovePartialNestedDirContent(self, snapshot, other):
        ...
        other.node.children["two"].children["nested"].children["one"].children["two"].children.pop(
            "file1"
        )
...
class TestDirHashPlaceholder:
    # ----------------------------------------------------------------------
    def test_Equal(self):
        assert DirHashPlaceholder(explicitly_added=True) == DirHashPlaceholder(
...
class TestNodeProperties:
    ...
    def test_Fullpath(self):
        ...
        assert (
            node.children["one"].children["two"].children["three"].fullpath
            == Path("one") / "two" / "three"
        )
